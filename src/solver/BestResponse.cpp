...
BestResponse::BestResponse(vector<vector<PrivateCards>> &private_combos,
  ...
  if (private_combos.size() != player_number)
    throw runtime_error(
        tfm::format("river combo length NE player nunber: %s -- %s",
                    private_combos.size(), player_number));
#endif
...
float BestResponse::printExploitability(shared_ptr<GameTreeNode> root,
  ...
  qDebug().noquote() << QString::fromStdString(tfm::format(
      QObject::tr("Iter: %s").toStdString().c_str(), iterationCount));
  // initialize the reach probs via the weights
  float exploitible = 0;
  ...
  for (int player_id = 0; player_id < this->player_number; player_id++) {
    ...
    qDebug().noquote() << (QString::fromStdString(tfm::format(
        QObject::tr("player %s exploitability %s").toStdString().c_str(),
        player_id, player_exploitability)));
  }
  float total_exploitability =
      exploitible / this->player_number / initial_pot * 100;
  qDebug().noquote() << QString::fromStdString(tfm::format(
      QObject::tr("Total exploitability %s precent").toStdString().c_str(),
...
vector<float> BestResponse::bestResponse(
    shared_ptr<GameTreeNode> node, int player,
...
vector<float> BestResponse::chanceBestReponse(
    shared_ptr<ChanceNode> node, int player,
    const vector<vector<float>> &reach_probs, uint64_t current_board,
    int deal) {
  ...
  // iterate through each of the licensing possibilities
...
#pragma omp parallel for
  ...
  for (int card = 0; card < node->getCards().size(); card++) {
...
#endif
      // TODO: change the color via offset
    ...
    if (child_utility.empty()) continue;
#ifdef DEBUG
...
vector<float> BestResponse::actionBestResponse(
    shared_ptr<ActionNode> node, int player,
    const vector<vector<float>> &reach_probs, uint64_t board, int deal) {
  if (player == node->getPlayer()) {
    // if you are making your own decisions, then definitely choose the most beneficial to you
    ...
    // if someone else is making the decision, then weight the calculation according to someone else's strategy
    ...
    for (int action_ind = 0; action_ind < node->getChildrens().size();
...
#ifdef DEBUG
      if (one_child == nullptr) throw runtime_error("child node not found");
#endif
      ...
      if (action_payoffs.size() != total_payoffs.size())
        throw runtime_error(
            tfm::format("length not match between action payoffs and total "
                        "payoffs %s -- %s",
...
vector<float> BestResponse::terminalBestReponse(
    shared_ptr<TerminalNode> node, int player,
    const vector<vector<float>> &reach_probs, uint64_t board, int deal) {
  uint64_t board_long = board;
  int oppo = 1 - player;
  const vector<RiverCombs> &player_combs =
      this->rrm.getRiverCombos(player, this->pcm.getPreflopCards(player),
                               board);
  const vector<RiverCombs> &oppo_combs = this->rrm.getRiverCombos(
      1 - player, this->pcm.getPreflopCards(1 - player),
      board);
...
vector<float> BestResponse::showdownBestResponse(
    shared_ptr<ShowdownNode> node, int player,
    const vector<vector<float>> &reach_probs, uint64_t board, int deal) {
#ifdef DEBUG
