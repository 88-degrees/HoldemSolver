...
int Card::getCardInt() { return this->card_int; }
...
int Card::card2int(Card card) { return strCard2int(card.getCard()); }
...
vector<int> Card::long2board(uint64_t board_long) {
  ...
  if (board.size() < 1 || board.size() > 7) {
    throw runtime_error(
        tfm::format("board length not correct, board length %s", board.size()));
  }
  return board;
}
...
string Card::toString() { return this->card; }
...
QString Card::toFormattedHtml() {
  ...
  if (qString.contains("c"))
    qString = qString.replace(
        "c",
        QString::fromLocal8Bit("<span style=\"color:black;\">&#9827;<\/span>"));
  else if (qString.contains("d"))
    qString = qString.replace(
        "d",
        QString::fromLocal8Bit("<span style=\"color:red;\">&#9830;<\/span>"));
